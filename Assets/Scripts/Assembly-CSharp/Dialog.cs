...
public class Dialog : MonoBehaviour
{
	...
	public bool cat_dialog;

	public string ach = string.Empty;
	...
	private void Update()
	{
		if (conv_started && !conv_ended && current_phrase < texts.Length)
		{
			...
			else if (Input.anyKeyDown)
			{
				...
				count = 0;
				tick_value = 0.05f;
				if (cat_dialog && current_phrase == 25)
				{
					Cat.GetComponent<Animator>().Play("cat_disappear");
				}
	...
	public void StartDialogAnyway()
	{
		if ((conv_started && conv_ended && translated) || !translated)
		{
			GetAchievement();
			GameObject.FindGameObjectWithTag("common").GetComponent<WorldScript>().DialogStarted();
			conv_started = true;
	...
	{
		if (col.gameObject.tag == "Player" && !conv_started && !conv_ended && data.story_progress < on_what_story_phase)
		{
			if (cat_dialog)
			{
				Cat.GetComponent<Animator>().Play("cat_appear");
			}
