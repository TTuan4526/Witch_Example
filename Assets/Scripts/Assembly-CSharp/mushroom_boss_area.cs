...
public class mushroom_boss_area : MonoBehaviour
{
	...
	private void Update()
	{
		...
		else if (current_phrase == texts.Length && !conv_ended && conv_started)
		{
			...
			conv_ended = true;
			boss.GetComponent<boss_mushroom>().Starter();
			finished = true;
	...
	public void Respawn()
	{
		GameObject[] ground_mushrooms = boss.GetComponent<boss_mushroom>().ground_mushrooms;
		foreach (GameObject gameObject in ground_mushrooms)
		{
			if (gameObject.name.Substring(9, 1) == "v")
			{
				gameObject.GetComponent<Animator>().Play("mush_violet_idle");
			}
			else if (gameObject.name.Substring(9, 1) == "r")
			{
				gameObject.GetComponent<Animator>().Play("mush_red_idle");
			}
			else if (gameObject.name.Substring(9, 1) == "y")
			{
				gameObject.GetComponent<Animator>().Play("mush_yellow_idle");
			}
		}
		GameObject[] right_mushrooms = boss.GetComponent<boss_mushroom>().right_mushrooms;
		foreach (GameObject gameObject2 in right_mushrooms)
		{
			if (gameObject2.name.Substring(9, 1) == "v")
			{
				gameObject2.GetComponent<Animator>().Play("mush_violet_idle");
			}
			else if (gameObject2.name.Substring(9, 1) == "r")
			{
				gameObject2.GetComponent<Animator>().Play("mush_red_idle");
			}
			else if (gameObject2.name.Substring(9, 1) == "y")
			{
				gameObject2.GetComponent<Animator>().Play("mush_yellow_idle");
			}
		}
		GameObject[] central_mushrooms = boss.GetComponent<boss_mushroom>().central_mushrooms;
		foreach (GameObject gameObject3 in central_mushrooms)
		{
			if (gameObject3.name.Substring(9, 1) == "v")
			{
				gameObject3.GetComponent<Animator>().Play("mush_violet_idle");
			}
			else if (gameObject3.name.Substring(9, 1) == "r")
			{
				gameObject3.GetComponent<Animator>().Play("mush_red_idle");
			}
			else if (gameObject3.name.Substring(9, 1) == "y")
			{
				gameObject3.GetComponent<Animator>().Play("mush_yellow_idle");
			}
		}
		GameObject[] left_mushrooms = boss.GetComponent<boss_mushroom>().left_mushrooms;
		foreach (GameObject gameObject4 in left_mushrooms)
		{
			if (gameObject4.name.Substring(9, 1) == "v")
			{
				gameObject4.GetComponent<Animator>().Play("mush_violet_idle");
			}
			else if (gameObject4.name.Substring(9, 1) == "r")
			{
				gameObject4.GetComponent<Animator>().Play("mush_red_idle");
			}
			else if (gameObject4.name.Substring(9, 1) == "y")
			{
				gameObject4.GetComponent<Animator>().Play("mush_yellow_idle");
			}
		}
