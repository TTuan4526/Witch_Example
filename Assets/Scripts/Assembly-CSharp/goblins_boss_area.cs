...
public class goblins_boss_area : MonoBehaviour
{
	public GameObject boss_fire;

	public GameObject boss_frost;
	...
	private int hp_fire = 12;

	private int hp_frost = 12;
	...
	private void Update()
	{
		...
		else if (current_phrase == texts.Length && !conv_ended && conv_started)
		{
			...
			conv_ended = true;
			boss_fire.GetComponent<boss_goblins>().Starter();
			boss_frost.GetComponent<boss_goblins>().Starter();
			finished = true;
	...
	public void UpdateHP(bool frost, int amount)
	{
		if (frost)
		{
			hp_frost = amount;
		...
		boss_hp.value = (float)(hp_frost + hp_fire) / 24f;
		if (hp_fire == 0 && hp_frost == 0)
		{
			BossDefeated();
		}
	}
	...
	public void CastCommonSpell()
	{
		boss_fire.GetComponent<boss_goblins>().CastSpell();
		boss_frost.GetComponent<boss_goblins>().CastSpell();
	}
	...
	public void SpikesAppear()
	{
		...
		{
			spike.GetComponent<cave_spike>().SpawnItemWithoutIdle();
			spike.gameObject.GetComponent<Animator>().Play("cave_spike_" + spike.GetComponent<cave_spike>().index + "_appear");
		}
		appeared = true;
		dropped = true;
	}

	public void FallToDefault()
	{
		fall = false;
		appeared = false;
	}

	public void SpikesDrop()
	{
		if (!fall)
		{
			StartCoroutine("SpikesDropCor");
			if (!dropped)
			{
				dropped = true;
			}
	...
	public void SpikesRestore()
	{
		foreach (Transform spike in spikes)
		{
			spike.gameObject.GetComponent<cave_spike>().SpawnItem();
		}
	}

	private IEnumerator SpikesDropCor()
	{
		...
		try
		{
			while (enumerator.MoveNext())
			{
				...
				spike.gameObject.GetComponent<cave_spike>().Fall();
			}
		}
		finally
		{
		...
		boss_fire.GetComponent<boss_goblins>().BackAfterSpell();
		boss_frost.GetComponent<boss_goblins>().BackAfterSpell();
	}
