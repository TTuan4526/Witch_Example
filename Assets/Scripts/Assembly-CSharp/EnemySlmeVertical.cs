...
public class EnemySlmeVertical : MonoBehaviour
{
	public Vector3 end;
	...
	public void SpawnMonster()
	{
		...
		facing_down = true;
		dead = false;
		anim.Play("enemy_slime_idle");
		GetComponent<BoxCollider2D>().enabled = true;
		current_speed = new Vector2(0f, -1.5f * speed);
	...
	private void Update()
	{
		if (!dead && !damaging && ((facing_down && base.transform.localPosition.y <= end.y) || (!facing_down && base.transform.localPosition.y >= end.y)))
		{
			temp_position = end;
			end = start;
			start = temp_position;
