...
public class cat_boss_area : MonoBehaviour
{
	...
	public Dialog after_battle_dialog;
	...
	private void Update()
	{
		...
		else if (current_phrase == texts.Length && !conv_ended && conv_started)
		{
			...
			conv_ended = true;
			boss.GetComponent<cat_boss>().Starter();
			finished = true;
	...
	public void Respawn()
	{
		...
		border.GetComponent<Animator>().Play("spikewall_idle");
		finished = false;
	...
	{
		border.GetComponent<BoxCollider2D>().enabled = false;
		border.GetComponent<Animator>().Play("spikewall_ungrow");
		hp_bar.SetActive(false);
		boss_name.SetActive(false);
		after_battle_dialog.StartDialog();
		data.story_progress = on_what_story_phase + 1;
		nimbus.GetComponent<end_nimbus>().FlyToThePoint();
		GameObject.Find("BossMusic").GetComponent<AudioSource>().Stop();
	...
	private void OnTriggerEnter2D(Collider2D col)
	{
		if (col.gameObject.tag == "Player" && data.story_progress < on_what_story_phase && !finished)
		{
			...
			border.GetComponent<Animator>().Play("spikewall_grow");
