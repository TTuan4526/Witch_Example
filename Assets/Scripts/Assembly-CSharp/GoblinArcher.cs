...
public class GoblinArcher : MonoBehaviour
{
	...
	{
		GetComponent<Animator>().Play("goblin_archer_idle");
		dead = false;
		damaging = false;
		facing_right = true;
		attacking = false;
	...
	private void ThrowArrow()
	{
		...
		{
			GameObject gameObject = Object.Instantiate(arrow, attack_position.transform.position, Quaternion.Euler(0f, 0f, 0f));
			gameObject.GetComponent<Rigidbody2D>().velocity = base.transform.right * -3.75f;
		}
		else if (base.transform.localScale.x == 1f)
		{
			GameObject gameObject2 = Object.Instantiate(arrow, attack_position.transform.position, Quaternion.Euler(0f, 0f, 180f));
	...
	public void PlayerEnterArea()
	{
		...
		{
			current_speed = new Vector2(0f, 0f);
			rbody.velocity = current_speed;
			if (fire_rate > 1f)
			{
				fire_rate = 1f;
	...
	public void DamageFromRight(int amount)
	{
		if (!dead)
		{
			...
			fire_rate = 0f;
			source.Play();
			anim.Play("goblin_archer_damage");
			damaging = true;
			rbody.velocity = new Vector2(0f, 0f);
		}
	}
	...
	private void Attack()
	{
		...
		attacking = true;
		rbody.velocity = new Vector2(0f, 0f);
		anim.Play("goblin_archer_attack");
	}
